...
// GET /api/v1/vehicles?keyword=xxx&departments=xxx&departments=xxx
// 根据关键词查询车牌信息（支持车主姓名和车牌号模糊查询），支持部门筛选
  ...
  try {
    ...
    if (keyword && keyword.trim() !== '') {
      ...
      filteredData = filteredData.filter(item =>
        item.owner_name.toLowerCase().includes(lowerKeyword) ||
        item.license_plate.toLowerCase().includes(lowerKeyword)
      );
    }
